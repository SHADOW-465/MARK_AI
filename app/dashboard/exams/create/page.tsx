...
export default function CreateExamPage() {
  ...
  const [passingPercentage, setPassingPercentage] = useState(35) // Default 35%
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase.from("exams").insert({
        ...
        passing_marks: Math.round((totalMarks * passingPercentage) / 100), // Calculate from percentage
        ...
        <Card className="border-border bg-card/40 backdrop-blur-md shadow-xl">
          ...
          <CardContent className="grid gap-6 md:grid-cols-2 p-6">
            ...
            <div className="grid gap-2">
              ...
              <p className="text-xs text-muted-foreground">
                Passing Marks: {Math.round((totalMarks * passingPercentage) / 100)} / {totalMarks}
              </p>
            </div>
