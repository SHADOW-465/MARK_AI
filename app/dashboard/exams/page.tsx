    ...
    <div className="space-y-8 pb-24 lg:pb-0">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm font-mono text-muted-foreground">Manage your exams, rubrics, and grading.</p>
        </div>
      ...
      <GlassCard className="p-0 overflow-hidden">
        <div className="p-6 border-b border-border">
          ...
          <p className="text-xs text-muted-foreground font-mono mt-1">A LIST OF ALL EXAMS CREATED BY YOU</p>
        </div>
          ...
          <table className="w-full text-left text-sm text-muted-foreground">
            ...
            <tbody className="divide-y divide-border">
              {exams && exams.length > 0 ? (
                exams.map((exam) => (
                  <tr key={exam.id} className="hover:bg-slate-100/50 dark:hover:bg-white/5 transition-colors group">
                    <td className="px-6 py-4 font-medium text-foreground">
                      <Link
                        href={`/dashboard/exams/${exam.id}`}
                        className="flex items-center gap-2 group-hover:text-cyan-600 dark:group-hover:text-cyan-400 transition-colors"
                      >
                        <div className="p-2 rounded-lg bg-slate-100 dark:bg-white/5 group-hover:bg-cyan-500/20 transition-colors">
                          <FileText className="h-4 w-4 text-muted-foreground group-hover:text-cyan-600 dark:group-hover:text-cyan-300" />
                        </div>
                        {exam.exam_name}
                      </Link>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <span className="inline-flex items-center rounded-md bg-purple-500/10 px-2 py-1 text-xs font-medium text-purple-700 dark:text-purple-300 border border-purple-500/20">
                        {exam.class}
                      </span>
                    </td>
                    ...
                    <td className="px-6 py-4 text-right">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="bg-card border-border text-card-foreground">
                          ...
                          <DropdownMenuItem asChild className="focus:bg-slate-100 dark:focus:bg-white/10 focus:text-foreground cursor-pointer">
